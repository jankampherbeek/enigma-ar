...
func TestCalculatePersephoneRam(t *testing.T) {
	...
	result := c.Calculate(domain.SePersephoneRam, jdUt, domain.ObsPosGeocentric)
...
func TestCalculateHermesRam(t *testing.T) {
	...
	c := NewPointsElementsCalculation()
	result := c.Calculate(domain.SeHermesRam, jdUt, domain.ObsPosGeocentric)
	if math.Abs(result[0]-expected) > 1e-8 {
...
func TestCalculateDemeterRam(t *testing.T) {
	...
	c := NewPointsElementsCalculation()
	result := c.Calculate(domain.SeDemeterRam, jdUt, domain.ObsPosGeocentric)
	if math.Abs(result[0]-expected) > 1e-8 {
